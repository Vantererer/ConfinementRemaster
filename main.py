...
character_name = "" # this is better than globa
items = []
...
def intro():
    print(character_name + ' wakes up in a confined, medium sized room. All that goes through their head is just how did they get there?')
    sleep(3)
    x = better_input("That doesn't matter, your goal is to help them get out of this weird area they are not famillar this, do you wish to cooperate? [y] [n] ",["y","y","n","n"])
    if match_from_array(x, ['y','y']):
        main()
        clean()
    elif match_from_array(x, ['n','n']):
...
def main():
    print(character_name+" stands in the center of the room, all thats around them is a vent, a bed, a desk with a laptop on it and a steel door with a keypad on it, what does "+character_name+" do?")
    ...
    if match_from_array(x, ['1','1']): # "Check Vent"
        ...
        x = better_input(character_name+" is at the desk, theres a laptop with power on and a few drawers, what do they do? [1] Check Laptop [2] Check the drawers [3] Go back ", ["1", "2", "3"])
